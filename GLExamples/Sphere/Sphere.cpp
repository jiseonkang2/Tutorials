// Sphere.cpp : fragment shading of sphere model
//
...
const int NumTriangles = 256;  // (4 faces)^(NumTimesToSubdivide + 1)
#endif
...
{
    vec3  normal = normalize(cross(vec3(b - a), vec3(c - b)));
...
void
divide_triangle(const vec4& a, const vec4& b,
...
display(void)
{
    ...
    vec3 eye(0.0, 0.0, 2.0);
    vec3 at(0.0, 0.0, 0.0);
    vec3 up(0.0, 1.0, 0.0);
    mat4 mv = lookAt(eye, at, up);

    glUniformMatrix4fv(ModelView, 1, GL_FALSE, value_ptr(mv));
